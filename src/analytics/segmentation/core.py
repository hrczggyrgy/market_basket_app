...
def calculate_segment_value_metrics(segments_df: pd.DataFrame, transactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    segment_metrics['customer_share_pct'] = (segment_metrics['customers'] / total_customers * 100) if total_customers > 0 else 0
    segment_metrics['revenue_share_pct'] = (segment_metrics['revenue'] / total_revenue * 100) if total_revenue > 0 else 0
    segment_metrics['transaction_share_pct'] = (segment_metrics['transactions'] / total_transactions * 100) if total_transactions > 0 else 0
    segment_metrics['unit_share_pct'] = (segment_metrics['units'] / total_units * 100) if total_units > 0 else 0
    ...
    segment_metrics['revenue_per_transaction'] = segment_metrics['revenue'] / segment_metrics['transactions']
...
def calculate_segment_engagement_metrics(segments_df: pd.DataFrame, transactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Merge segment info with transaction data
    df = transactions_df.merge(segments_df[['customer_id', 'segment']], on='customer_id', how='left')
    df['date'] = pd.to_datetime(df['date'])
    
    # Calculate engagement metrics per customer first
    customer_engagement = df.groupby(['customer_id', 'segment']).agg(
        purchase_frequency=('transaction_id', 'nunique'),
        recency_days=('date', lambda x: (df['date'].max() - x.max()).days),
        avg_days_between=('date', lambda x: (x.max() - x.min()).days / max(x.nunique() - 1, 1) if x.nunique() > 1 else 0),
    ...
    segment_metrics = customer_engagement.groupby('segment').agg(
    ...
    active_customers = df[df['date'] >= thirty_days_ago].groupby('segment')['customer_id'].nunique().reset_index()
    ...
    dormant_customers = df[df['date'] < ninety_days_ago].groupby('segment')['customer_id'].nunique().reset_index()
    ...
    return engagement_metrics[['segment', 'purchase_frequency', 'recency_days', 'avg_days_between', 
                             'active_weeks', 'active_months', 'purchase_streak', 
                             'active_customer_rate_pct', 'dormancy_rate_pct']]
...
def calculate_segment_retention_metrics(transactions_df: pd.DataFrame, segments_df: pd.DataFrame) -> pd.DataFrame:
    ...
    customer_lifecycle = df.groupby(['customer_id', 'segment']).agg(
    ...
    repeat_customers = customer_lifecycle.groupby('segment')['is_repeat_customer'].mean().reset_index()
    repeat_customers.columns = ['segment', 'repeat_purchase_rate_pct']
    repeat_customers['repeat_purchase_rate_pct'] *= 100
    
    # Aggregate to segment level
    segment_metrics = customer_lifecycle.groupby('segment').agg(
    ...
    active_customers = df[df['date'] >= sixty_days_ago].groupby('segment')['customer_id'].nunique().reset_index()
    active_customers.columns = ['segment', 'active_customers']
    ...
    lifecycle_metrics['lapse_rate_pct'] = (
        ...
        lifecycle_metrics['total_customers'] * 100
    ).fillna(0)
    ...
    return lifecycle_metrics[['segment', 'customer_lifetime_days', 'tenure', 
                            'repeat_purchase_rate_pct', 'lapse_rate_pct', 'reactivation_rate_pct']]


def calculate_segment_basket_metrics(segments_df: pd.DataFrame, transactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    basket_metrics = df.groupby(['customer_id', 'segment', 'transaction_id']).agg(
        basket_value=('revenue', 'sum'),
        basket_units=('quantity', 'sum'),
        basket_skus=('stockcode', 'nunique'),
        basket_categories=('category', 'nunique') if 'category' in df.columns else ('stockcode', lambda x: 1)  # Fallback if no category
    ).reset_index()
    ...
    # Aggregate to segment level
    segment_metrics = basket_metrics.groupby('segment').agg(
    ...
    basket_rates = basket_metrics.groupby('segment').agg(
    ...
    return segment_metrics[['segment', 'avg_basket_value', 'avg_units_per_basket', 
                          'avg_skus_per_basket', 'avg_categories_per_basket',
                          'basket_concentration_gini', 'basket_diversity_entropy',
                          'large_basket_rate_pct', 'small_basket_rate_pct']]


def calculate_segment_price_behavior_metrics(segments_df: pd.DataFrame, transactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    overall_price_std = df['price'].std()
    
    # Calculate price metrics per customer
    customer_price_metrics = df.groupby(['customer_id', 'segment']).agg(
    ...
    customer_price_metrics['price_cv'] = customer_price_metrics['price_std'] / customer_price_metrics['avg_price_paid']
    customer_price_metrics['price_cv'] = customer_price_metrics['price_cv'].fillna(0)
    customer_price_metrics['price_range'] = customer_price_metrics['max_price'] - customer_price_metrics['min_price']
    
    # Aggregate to segment level
    segment_metrics = customer_price_metrics.groupby('segment').agg(
    ...
    premium_share = df.groupby(['customer_id', 'segment'])['price'].apply(_calculate_premium_share).reset_index()
    premium_share.columns = ['customer_id', 'segment', 'premium_product_share_pct']
    
    value_share = df.groupby(['customer_id', 'segment'])['price'].apply(_calculate_value_share).reset_index()
    value_share.columns = ['customer_id', 'segment', 'value_product_share_pct']
    
    segment_premium = premium_share.groupby('segment')['premium_product_share_pct'].mean().reset_index()
    ...
    return segment_metrics[['segment', 'avg_price_paid', 'price_cv', 'price_range',
                          'price_index_vs_overall', 'price_orientation_index',
                          'premium_product_share_pct', 'value_product_share_pct', 'kvi_penetration_pct']]


def calculate_segment_growth_metrics(transactions_df: pd.DataFrame, segments_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_period_metrics(period_df, period_segments_df):
        ...
        merged = period_df.merge(period_segments_df[['customer_id', 'segment']], on='customer_id', how='left')
        
        if merged.empty or 'segment' not in merged.columns:
            return pd.DataFrame()
        
        metrics = merged.groupby('segment').agg(
            revenue=('revenue', 'sum'),
            customers=('customer_id', 'nunique'),
            transactions=('transaction_id', 'nunique'),
            avg_order_value=('revenue', 'mean')
    ...
    return growth_metrics[['segment', 'revenue_growth_pct', 'customer_growth_pct', 
                          'frequency_growth_pct', 'avg_order_value_growth_pct', 
                          'segment_share_change_pct']]


def calculate_segment_concentration_metrics(segments_df: pd.DataFrame, transactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    revenue_stats = df.groupby('segment')['revenue'].agg(
        revenue_mean='mean',
    ...
    return concentration_metrics[['segment', 'revenue_concentration_gini', 
                               'top_1pct_revenue_share', 'top_5pct_revenue_share', 
                               'top_10pct_revenue_share', 'revenue_mean', 'revenue_median', 'revenue_std']]


def calculate_segment_stability_score(segments_df: pd.DataFrame, transactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    segment_metrics = segments_df.groupby('segment').agg(
    ...
    if max_customers > 0:
        segment_metrics['size_score'] = np.log(segment_metrics['customer_count'] + 1) / np.log(max_customers + 1)
    else:
...
def calculate_segment_distinctiveness(segments_df: pd.DataFrame, transactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    distinctiveness_df = pd.DataFrame({
        'segment': segment_means.index,
        'distinctiveness_score': distances
    })
    ...
    if distinctiveness_df['distinctiveness_score'].max() > 0:
        distinctiveness_df['distinctiveness_score'] = (
            distinctiveness_df['distinctiveness_score'] / distinctiveness_df['distinctiveness_score'].max() * 100
        )
    else:
    ...
    for segment in segment_means.index:
        ...
        z_scores = ((segment_mean - overall_means) / (overall_std := overall_means.std() + 1e-10)).abs()
