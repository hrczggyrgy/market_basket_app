...
def detect_promotions(
    ...
    """Detect potential promo periods from price drops vs per-SKU 90th-percentile baseline.

    WARNING: This is a HEURISTIC price-drop detection, NOT verified promotion identification.
    It identifies periods where price drops >= threshold vs the SKU's 90th-percentile price.
    Results may include: clearance, permanent price changes, competitive matching, markdowns,
...
def compute_promo_baseline(
    ...
    The baseline is fitted ONLY on non-promotional weeks. For promotional weeks,
    the baseline is forward-filled from the last observed non-promotional
    trend/seasonal components to avoid leakage from post-promotional data.
    """
    df = df.copy()
    df["date"] = pd.to_datetime(df["date"])
    df["revenue"] = df["price"] * df["quantity"]
    ...
    results = []
    for _stockcode, sku in weekly.groupby("stockcode", sort=False):
        ...
        if len(sku) >= seasonal_period * 2 and non_promo_mask.sum() >= seasonal_period:
            try:
                # Fit STL ONLY on non-promo data (no interpolation across promo weeks)
                non_promo_sku = sku[non_promo_mask].copy()
                non_promo_sku = non_promo_sku.sort_values("week").reset_index(drop=True)

                units_non_promo = non_promo_sku["actual_units"].to_numpy(dtype=float)
                revenue_non_promo = non_promo_sku["actual_revenue"].to_numpy(dtype=float)

                # Fit STL on non-promo data only
                stl_units = STL(units_non_promo, seasonal=seasonal_period, robust=True).fit()
                stl_revenue = STL(revenue_non_promo, seasonal=seasonal_period, robust=True).fit()

                # Get trend + seasonal for non-promo weeks
                trend_units = stl_units.trend
                seasonal_units = stl_units.seasonal
                trend_revenue = stl_revenue.trend
                ...
                promo_indices = np.where(~non_promo_mask)[0]
                for idx in promo_indices:
                    # Find last non-promo index before this promo week
                    prior_non_promo = non_promo_indices[non_promo_indices < idx]
                    if len(prior_non_promo) > 0:
                        last_idx = prior_non_promo[-1]
                        baseline_units[idx] = baseline_units[last_idx]
                        baseline_revenue[idx] = baseline_revenue[last_idx]
                    else:
                        # No prior non-promo data, use first non-promo value
                        first_idx = non_promo_indices[0] if len(non_promo_indices) > 0 else 0
                        baseline_units[idx] = baseline_units[first_idx]
...
def compute_incrementality_waterfall(
    ...
    """Waterfall: baseline + incremental (qty + price) + halo - cannibalization = net incremental.

    This is the DESCRIPTIVE waterfall. For causal incrementality, use the causal engine.
    """
    agg = (
        ...
        .agg(
            ...
            incremental_revenue_qty=("incremental_revenue_qty", "sum"),
            incremental_revenue_price=("incremental_revenue_price", "sum"),
        )
        .reset_index()
    )
    ...
    for col in ("stockpiling_revenue",):
