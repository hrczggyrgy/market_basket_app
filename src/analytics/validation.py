...
class ValidationHarness:
    ...
    def _run(self, module: str, function: str, fn: Callable[..., Any], *args: Any, **kwargs: Any) -> ValidationResult:
        ...
        try:
            ...
            if isinstance(output, tuple):
                ...
                for i, o in enumerate(output[1:]):
                    if isinstance(o, dict):
                        ...
                        diagnostics[f"output_{i}"] = {"shape": o.shape, "columns": o.columns.tolist()}
            else:
                main_output = output
                diagnostics = {}
            ...
            return ValidationResult(
                ...
                output_columns=main_output.columns.tolist() if isinstance(main_output, pd.DataFrame) else None,
    ...
    def run_all(self) -> List[ValidationResult]:
        ...
        def _run_and_store(module: str, function: str, fn: Callable[..., Any], *args: Any, **kwargs: Any) -> ValidationResult:
        ...
        _run_and_store("switching", "get_customer_loyalty_metrics", get_customer_loyalty_metrics, self.df)
        ...
        _run_and_store("basket_metrics", "compute_basket_penetration", compute_basket_penetration, self.df)
        _run_and_store("basket_metrics", "compute_customer_entropy", compute_customer_entropy, self.df)
        ...
        if not pm.empty:
            ...
            _run_and_store("performance", "product_lifecycle_stage", product_lifecycle_stage, self.df)
            ...
            _run_and_store("performance", "compute_time_to_second_purchase", compute_time_to_second_purchase, self.df)
        ...
        _run_and_store("category", "compute_category_scorecard", compute_category_scorecard, self.df)

        # Choice model
        from src.analytics.choice_model import build_customer_features, train_choice_model
        cf = _run_and_store("choice_model", "build_customer_features", build_customer_features, self.df)
        if cf.success:
            _run_and_store("choice_model", "train_choice_model", train_choice_model, cf.diagnostics.get("output", cf.output_shape and self.df))
        ...
        if dt.success:
            ...
            if dt_output is not None:
                _run_and_store("transference", "compute_substitutable_demand_percentage", compute_substitutable_demand_percentage, dt_output, self.df)
                _run_and_store("transference", "delist_impact_analysis", delist_impact_analysis, self.df, dt_output, dt_output["from_product"].unique()[:5])
        ...
        if attrs.success:
            ...
            if sim.success:
                _run_and_store("cdt", "get_cluster_assignments", get_cluster_assignments, self.df, sim.diagnostics.get("output", sim.output_shape and self.df))
                tree = _run_and_store("cdt", "build_cdt", build_cdt, attrs.diagnostics.get("output", attrs.output_shape and self.df), sim.diagnostics.get("output", sim.output_shape and self.df))
                if tree.success:
                    _run_and_store("cdt", "tree_to_dataframe", tree_to_dataframe, tree.diagnostics.get("output", tree.output_shape and self.df))
        ...
        if rfm.success:
            _run_and_store("segmentation", "rfm_segmentation", rfm_segmentation, rfm.diagnostics.get("output", rfm.output_shape and self.df))
        _run_and_store("segmentation", "behavioral_segmentation", behavioral_segmentation, self.df)
        _run_and_store("segmentation", "value_based_segmentation", value_based_segmentation, self.df)
    ...
    def print_summary(self) -> None:
        ...
        for _, row in df.iterrows():
            ...
            print(f"  {status} {row['module']}.{row['function']}: {row['output_shape']} {row['error'] or ''}")
...
def assert_validation(harness: ValidationHarness, baseline_path: str) -> List[str]:
    ...
    if not os.path.exists(baseline_path):
        raise FileNotFoundError(f"Baseline not found: {baseline_path}. Run export_baseline() first.")
