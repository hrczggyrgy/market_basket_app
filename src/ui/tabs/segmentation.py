...
def _segment_revenue_share(seg: pd.DataFrame, value_col: str, label_col: str) -> None:
    ...
    show(fig)
    st.caption(
        "Share of total customer value per segment. A concentrated pie = the base is fragile."
    )
...
def _segment_migration(df: pd.DataFrame) -> None:
    ...
    if migration.empty:
        st.caption(
            "Not enough stable segments to trace migration (customers must be present in both halves)."
        )
        return
...
def render(df: pd.DataFrame) -> None:
    ...
    with tab1:
        ...
        with col1:
            st.dataframe(
                seg[["customer_id", "recency_days", "frequency", "monetary", "segment"]],
    ...
    with tab2:
        ...
        with col1:
            st.dataframe(
                behav[["customer_id", "cluster", "segment", "cluster_confidence"]],
                use_container_width=True,
                hide_index=True,
            )
        with col2:
        ...
        try:
            ...
            with st.expander("���💰 Segment Value & Economic Importance", expanded=True):
                ...
                if not value_metrics.empty:
                    ...
                    display_metrics["revenue"] = display_metrics["revenue"].apply(
                        lambda x: f"€{x:,.0f}"
                    )
                    display_metrics["revenue_per_customer"] = display_metrics[
                        "revenue_per_customer"
                    ].apply(lambda x: f"€{x:,.0f}")
                    display_metrics["revenue_per_transaction"] = display_metrics[
            ...
            with st.expander("���👥 Customer Engagement", expanded=True):
                ...
                if not engagement_metrics.empty:
                    ...
                    display_metrics["active_customer_rate_pct"] = display_metrics[
                        "active_customer_rate_pct"
                    ].apply(lambda x: f"{x:.1f}%")
                    display_metrics["dormancy_rate_pct"] = display_metrics[
                        "dormancy_rate_pct"
                    ].apply(lambda x: f"{x:.1f}%")
                    st.dataframe(
                        display_metrics[
            ...
            with st.expander("���🔄 Retention & Lifecycle", expanded=True):
                ...
                if not retention_metrics.empty:
                    ...
                    display_metrics["repeat_purchase_rate_pct"] = display_metrics[
                        "repeat_purchase_rate_pct"
                    ].apply(lambda x: f"{x:.1f}%")
                    display_metrics["lapse_rate_pct"] = display_metrics["lapse_rate_pct"].apply(
                        lambda x: f"{x:.1f}%"
                    )
                    display_metrics["reactivation_rate_pct"] = display_metrics[
            ...
            with st.expander("���🛒 Basket Economics", expanded=True):
                ...
                if not basket_metrics.empty:
                    ...
                    display_metrics["avg_basket_value"] = display_metrics["avg_basket_value"].apply(
                        lambda x: f"€{x:,.2f}"
                    )
                    display_metrics["large_basket_rate_pct"] = display_metrics[
                        "large_basket_rate_pct"
                    ].apply(lambda x: f"{x:.1f}%")
                    display_metrics["small_basket_rate_pct"] = display_metrics[
            ...
            with st.expander("���💵 Price Behavior", expanded=True):
                ...
                if not price_metrics.empty:
                    ...
                    display_metrics["avg_price_paid"] = display_metrics["avg_price_paid"].apply(
                        lambda x: f"€{x:.2f}"
                    )
                    display_metrics["price_orientation_index"] = display_metrics[
                        "price_orientation_index"
                    ].apply(lambda x: f"{x:+.1f}")
                    display_metrics["premium_product_share_pct"] = display_metrics[
                        "premium_product_share_pct"
                    ].apply(lambda x: f"{x:.1f}%")
                    display_metrics["value_product_share_pct"] = display_metrics[
            ...
            with st.expander("���📈 Growth & Momentum", expanded=True):
                ...
                if not growth_metrics.empty:
                    ...
                    display_metrics["revenue_growth_pct"] = display_metrics[
                        "revenue_growth_pct"
                    ].apply(lambda x: f"{x:+.1f}%")
                    display_metrics["customer_growth_pct"] = display_metrics[
                        "customer_growth_pct"
                    ].apply(lambda x: f"{x:+.1f}%")
                    display_metrics["frequency_growth_pct"] = display_metrics[
                        "frequency_growth_pct"
                    ].apply(lambda x: f"{x:+.1f}%")
                    display_metrics["avg_order_value_growth_pct"] = display_metrics[
                        "avg_order_value_growth_pct"
                    ].apply(lambda x: f"{x:+.1f}%")
                    display_metrics["segment_share_change_pct"] = display_metrics[
            ...
            with st.expander("���🎯 Customer Concentration", expanded=True):
                ...
                if not concentration_metrics.empty:
                    ...
                    display_metrics["top_10pct_revenue_share"] = display_metrics[
            ...
            with st.expander("��✨ Segment Quality", expanded=True):
                ...
                if not stability_metrics.empty:
                    ...
                    display_stability["stability_score"] = display_stability[
                        "stability_score"
                    ].apply(lambda x: f"{x:.1f}")
                    st.dataframe(
                        display_stability[
                ...
                if not distinctiveness_metrics.empty:
                    ...
                    display_distinct["distinctiveness_score"] = display_distinct[
