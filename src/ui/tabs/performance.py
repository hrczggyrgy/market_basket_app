...
def render(df: pd.DataFrame) -> None:
    ...
    basket_insights = generate_basket_insights(df)
    # Combine insights
    all_insights = []
    if not insights.empty:
        all_insights.append(insights)
    if not basket_insights.empty:
        all_insights.append(basket_insights)
    if all_insights:
        combined_insights = pd.concat(all_insights, ignore_index=True)
    else:
        combined_insights = pd.DataFrame()
    render_insight_cards(combined_insights)

    st.divider()
    st.subheader(":material/task_alt: Ranked Decisions")
    product_opps = generate_product_opportunities(rational, xyz, lifecycle)
    basket_opps = generate_basket_opportunities(df)
    # Combine opportunities
    all_opps = []
    if not product_opps.empty:
        all_opps.append(product_opps)
    if not basket_opps.empty:
        all_opps.append(basket_opps)
    if all_opps:
        combined_opps = pd.concat(all_opps, ignore_index=True)
    else:
        combined_opps = pd.DataFrame()
    render_opportunity_cards(combined_opps)
