...
def test_compute_switching_status_basic() -> None:
    ...
    df = _create_sample_switching_data()
    status = compute_switching_status(
        df, window_days=90, min_transactions=2, min_customers=1, min_transitions=1
    )
...
def test_compute_switching_status_edge_cases() -> None:
    ...
    status = compute_switching_status(
        df_low_obs, window_days=90, min_transactions=5, min_customers=1, min_transitions=1
    )
    ...
    status = compute_switching_status(
        df_low_cust, window_days=90, min_transactions=1, min_customers=2, min_transitions=1
    )
...
def test_compute_substitution_strength() -> None:
    ...
    try:
        ...
        if not substitution.empty:
            ...
            assert (substitution["switch_rate"] >= 0).all() and (
                substitution["switch_rate"] <= 1
            ).all()
            assert (substitution["switch_rate_ci_lower"] >= 0).all() and (
                substitution["switch_rate_ci_lower"] <= 1
            ).all()
            assert (substitution["switch_rate_ci_upper"] >= 0).all() and (
                substitution["switch_rate_ci_upper"] <= 1
            ).all()
            assert (substitution["confidence"] >= 0).all() and (
                substitution["confidence"] <= 1
            ).all()
...
def test_compute_high_value_switching() -> None:
    ...
    try:
        high_value = compute_high_value_switching(
            df, demand_transference_df, clv_customer_df, top_n_segments=2
        )
...
def test_switching_functions_with_real_data(sample_df: pd.DataFrame) -> None:
    ...
    status = compute_switching_status(
        sample_df, window_days=90, min_transactions=3, min_customers=5, min_transitions=3
    )
    SWITCHING_STATUS.validate(status)
    ...
    if not matrix.empty:
        ...
        demand_transference_df = pd.DataFrame(
            {
                "from_product": matrix["from_product"].head(2).tolist()
                if len(matrix) >= 2
                else ["A", "B"],
                "to_product": matrix["to_product"].head(2).tolist()
                if len(matrix) >= 2
                else ["B", "A"],
        ...
        try:
            ...
            high_value = compute_high_value_switching(
                sample_df, demand_transference_df, high_value_df
            )
                ...
                pd.DataFrame(
                    {
                        "from_product": ["A"],
                        "to_product": ["B"],
                        "substitution_strength": ["moderate"],
                    }
                ),
...
def test_switching_functions_consistency() -> None:
    ...
    if not matrix.empty:
        ...
        insufficient_obs = status_for_switching[
            status_for_switching["switching_status"] == "insufficient_observations"
        ]
